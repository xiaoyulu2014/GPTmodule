...
r = 20;n = 150;Q = 200;seed = 17;m = 50; L = 10;
...
title("NHT_SGLD, traceplots of w, \n n = 150, Q = 200, r = 20, batchsize = 50")
subplot(122)
plot(collect(U_store[1,1,1,:]));plot(collect(U_store[2,2,2,:]));plot(collect(U_store[3,3,3,:]));
title("NHT_SGLD, traceplots of U, \n n = 150, Q = 200, r = 20, batchsize = 50")
savefig("NHT_SGLD_m50")
=#


using HDF5
c=h5open("res_batchsize50.h5","w") do file
	write(file,"RMSEtest50",RMSEtest);
	write(file,"RMSEtrain50",RMSEtrain);
	write(file,"timer_learn50",timer_learn);
	write(file,"timer_test50",timer_test);
	write(file,"RMSEtrainve50",RMSEtrainvec);
	write(file,"RMSEtestvec50",RMSEtestvec);
end
